...
- `NTRIP-07`: 有効なGGAがある接続でRTCMを30秒受信できなければストリームを切断し、3秒後を3回、5秒後を3回、10秒後を3回、20秒後を3回、30秒後を6回、その後は60秒ごとに再接続すること。RTCMを30秒安定受信した後だけ連続失敗回数をリセットし、検証済みネットワークの復旧通知では待機中の再試行を直ちに起こすこと。認証・設定・TLSの恒久エラーおよびユーザー切断では自動再試行しないこと。
...
- `NET-01`: Map上部のInternet表示は、default networkが存在するだけではOnlineとせず、Androidが`NET_CAPABILITY_INTERNET`と`NET_CAPABILITY_VALIDATED`を確認した場合だけ緑とすること。経路はあるが未検証の場合は橙、経路がない場合は灰とすること。

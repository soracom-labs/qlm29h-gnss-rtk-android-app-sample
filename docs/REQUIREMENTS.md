...
- `NTRIP-07`: 有効なGGAがある接続でRTCMを30秒受信できなければストリームを切断し、1、2、4、8、16、32、最大60秒を基準とする±20%のjitter付き指数バックオフで再接続すること。RTCMを30秒安定受信した後だけ連続失敗回数をリセットし、ネットワーク復旧通知では待機中の再試行を直ちに起こすこと。認証・設定・TLSの恒久エラーおよびユーザー切断では自動再試行しないこと。

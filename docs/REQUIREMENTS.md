...
- `SORACOM-05`: Unified Endpointへの定期送信間隔は3、5、6、10、15、30、60秒の選択式とし、初期値は60秒とすること。60秒以外へ変更する際は、通信量と料金の試算を促す確認ダイアログを表示し、明示的な確認後だけ適用すること。不正値および確認機構導入前の保存値は60秒へフォールバックすること。

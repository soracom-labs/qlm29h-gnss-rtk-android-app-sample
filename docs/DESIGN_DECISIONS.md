...
再接続は1秒から最大60秒の指数バックオフへ±20%のjitterを加える。接続直後の切断ループで接続頻度が1秒へ戻らないよう、RTCMを30秒安定受信した後だけ連続失敗回数をリセットする。ネットワーク復旧は待機を解除するが、再接続Jobそのものは`NtripSessionController`だけが所有する。認証、Mount Point、TLSなど利用者の修正が必要なエラーは停止し、無限再試行しない。

...
## DD-21 正確な位置履歴をAndroidバックアップへ含めない

Room DB、Smartphone GNSS、NMEA/RTCM生ログには正確な走行位置と時刻が含まれ、NTRIP設定には接続情報が含まれる。利用者が明示的に共有していない状態でクラウドバックアップや端末間移行へ複製されることを避けるため、アプリ全体のAndroidバックアップを無効にする。

セッションの持ち出しはアプリ内のShare操作を唯一の経路とし、利用者が共有先を選択できる状態を維持する。バックアップを無効にしても、一部端末メーカーの独自移行機能まで制御できるとはみなさず、READMEでアンインストール・端末交換前の明示的なエクスポートを案内する。

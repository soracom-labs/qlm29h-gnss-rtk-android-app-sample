...
本書は、社内評価用APK、公開評価用APK、公開サンプルAPK、製品としての正式配布を混同せず、安全に配布するための手順を定める。本リポジトリから配布するAPKは、明示的な別承認がない限り検証用サンプルであり、公式製品または公式サポート対象ではない。
...
| 公開評価版 | GitHub利用者による初期検証 | Androidのデバッグ鍵 | PublicリポジトリのGitHub Pre-release |
...
5. 社内評価版では信頼できる社内経路、公開評価版ではPublicリポジトリのPre-releaseからURLを案内する。

Publicリポジトリへ添付したRelease Assetは、Pre-releaseであっても一般にダウンロードできる。デバッグ署名APKをPublicリポジトリへ置く場合は、評価・検証専用でデバッグ可能なビルドであること、正式版またはGoogle Play向けではないこと、同じデバッグ鍵を失うと更新できないことをRelease Notesへ明記する。公開評価版をStable Releaseまたは製品版として扱わない。デバッグ鍵はリポジトリへ含めず、異なる鍵へ移行する場合は上書きできない旨とデータのエクスポート手順を案内する。
...
組織鍵で署名する公開サンプル版では、上記に加えて`PUBLIC_RELEASE_CHECKLIST.md`を完了する。デバッグ署名の公開評価版は必ずPre-releaseとし、上記の評価版向け注意事項を適用する。フィールド試験が残る変更を含む場合、ReleaseをStableとして公開せず、試験完了後に対象コミットと結果を確定する。

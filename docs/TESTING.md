...
18. RTCMを30秒以上停止させ、StaleからReconnectingへ移行し、再接続待機が3秒×3回、5秒×3回、10秒×3回、20秒×3回、30秒×6回、その後60秒ごとになることを確認する。検証済みネットワークの復旧時は待機中でも再接続し、30秒安定受信後の次回障害が初回の3秒へ戻ることを確認する。
19. default networkが存在しない場合はInternetが灰、`INTERNET` capabilityはあるが未検証の場合は橙、`VALIDATED`取得後だけ緑になることを確認する。Wi-FiやVPNのtransportが残っているだけで緑を維持しないことも確認する。

...
本プロジェクトは、RTK/GNSSの検証と用途別カスタマイズの出発点として公開しているサンプルアプリです。QuectelまたはSORACOMの公式アプリ・製品サポート対象ではなく、一般消費者向け製品、測量成果を保証する機器、カーナビゲーションまたは運転支援システムでもありません。
...
本リポジトリからAPKを作成・配布する担当者は、[APK配布ガイド](docs/DISTRIBUTION.md)と[公開リリースチェックリスト](docs/PUBLIC_RELEASE_CHECKLIST.md)に従ってください。
...
Map上部の`Internet`は、AndroidがVPNを除くWi-Fiやモバイル回線などの外部到達性を検証できた場合だけ緑になります。通信経路はあるものの検証中の場合は橙、経路がない場合は灰です。SORACOM ArcのVPNだけが残った状態をInternet接続とはみなしません。NTRIPとSORACOMの個別疎通は、それぞれ隣のステータスで確認してください。
...
- 本ソフトウェアの利用、測位誤差、通信断、ログ欠損または外部サービス停止により生じる判断や損害については、適用されるライセンス、契約および法令に従います。

公開サンプルAPKの作成者は、公開前に配布主体の法務・セキュリティ・プライバシー確認を完了してください。
...
- `docs/PUBLIC_RELEASE_CHECKLIST.md`: 公開サンプルAPKのリリース判定
- `CONTRIBUTING.md`: Issue・Pull Requestを送る際の注意
- `SECURITY.md`: 脆弱性を非公開で報告する方法
...
組織管理の鍵で署名した公開サンプルAPKを生成する場合は、リポジトリ外の鍵を`keystore.properties`から参照したうえで次を実行します。
